...
struct ImGuiIO
{
    ...
    ImVec2        DisplayFrameTextureBufferScale;  // = (1.0f,1.0f)        // For retina display or other situations where window coordinates are different from FrameTextureBuffer coordinates. User storage only, presently not used by ImGui.
...
struct ImDrawData
{
    ...
    IMGUI_API void ScaleClipRects(const ImVec2& sc);  // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scale than ImGui expects, or if there is a difference between your window resolution and FrameTextureBuffer resolution.
};
