...
namespace Examples
{
    class Program
    {
        ...
        {
            if (args.Length < 2)
                Console.WriteLine("Usage Reporter.exe [input data] [output docx]");
            // Read content
            ...
            if(lines.Length>1)
            {
                ...
                try 
                {
                    DocX document = DocX.Create(args[1]);
                    for (int i=0; i<lines.Length; ++i)
                    {
                        string line = lines[i];
                        if (line.Contains("title"))
                            InsertTitle(document, line);
                        else if (line.Contains("paragraph"))
                            InsertParagraph(document, line);
                        else if (line.Contains("table begin"))
                            i = InsertTable(document, lines, i);
                    }
                    document.Save();
                }
                finally
        ...
        static void InsertTitle(DocX document, string line)
        {
            string[] strs = line.Split('\t');
            if(strs.Length == 3) {
                Paragraph title = document.InsertParagraph().Append(strs[2])
        ...
        static void InsertParagraph(DocX document, string line)
        {
            string[] strs = line.Split('\t');
            if(strs.Length == 2)
            {
                Paragraph title1221 = document.InsertParagraph(strs[1])
                    .Font(new FontFamily("宋体")).FontSize(15);
            }
        }

        static int InsertTable(DocX document, string[] lines, int startline)
        {

            if (lines[startline].Contains("table begin"))
                startline++;
            int endline = startline;
            int cols = 0;
            for (int i = startline; i < lines.Length; ++i)
            {
                if (lines[i].Contains("table end"))
                {
                    endline = i;
                    break;
                }
                string[] substrings = lines[i].Split('\t');
                if (cols < substrings.Length)
                    cols = substrings.Length;
            }
            if (endline <= startline)
                return startline + 1;
            Table table = document.AddTable(endline-startline, cols);
            table.Design = TableDesign.TableGrid;
            table.Alignment = Alignment.center;
            for (int i= startline; i < endline; ++i)
            {
                string[] substrings = lines[i].Split('\t');
                int rowIdx = i - startline;
                for (int j = 0; j < substrings.Length; ++j)
                    table.Rows[rowIdx].Cells[j].Paragraphs[0].Append(substrings[j]);
            }
            document.InsertParagraph().Append(" ").InsertTableAfterSelf(table);
